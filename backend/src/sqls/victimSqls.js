...
const GET_VICTIM = `SELECT
    ...
    s.name AS shelter_name,
    s.capacity AS shelter_capacity,
    COALESCE((SELECT COUNT(*) FROM victims occupied WHERE occupied.shelter_id = s.shelter_id), 0) AS shelter_occupied,
    CASE
      WHEN s.shelter_id IS NULL THEN 'unassigned'
      WHEN (SELECT COUNT(*) FROM victims occupied WHERE occupied.shelter_id = s.shelter_id) >= s.capacity THEN 'full'
      ELSE 'available'
    END AS shelter_availability
  FROM victims v
  JOIN disasters d ON d.disaster_id = v.disaster_id
...
const LOCK_SHELTER = `SELECT shelter_id, capacity FROM shelters WHERE shelter_id = $1 FOR UPDATE`;
const COUNT_SHELTER_VICTIMS = `SELECT COUNT(*)::int AS occupied
  FROM victims
  WHERE shelter_id = $1 AND victim_id <> $2`;
...
const FIND_VICTIM = `SELECT victim_id, shelter_id FROM victims WHERE victim_id = $1 FOR UPDATE`;
...
const DELETE_VICTIM = `DELETE FROM victims WHERE victim_id = $1 RETURNING victim_id`;
