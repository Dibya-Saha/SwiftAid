...
async function createDistribution(req, res) {
  ...
  if (
    [requestId, warehouseId, teamId].some((value) => value === null) ||
    !Array.isArray(rawItems) ||
    !rawItems.length
  ) {
    return res.status(400).json({
      message: "request_id, warehouse_id, team_id, and items are required",
    });
  }
  ...
  try {
    ...
    if (
      !["approved", "waiting_stock", "partially_fulfilled"].includes(
        String(request.rows[0].status).toLowerCase(),
      )
    ) {
      await client.query("ROLLBACK");
      return res
        .status(400)
        .json({ message: "Relief request is not ready for distribution" });
    }
    ...
    for (const raw of rawItems) {
      ...
      if (!stock.rows[0]) {
        ...
        return res
          .status(409)
          .json({ message: "Warehouse does not have enough stock" });
      }
      items.push({
    ...
    const distribution = await client.query(sql.CREATE_DISTRIBUTION, [
      requestId,
      warehouseId,
      teamId,
      req.user.user_id,
    ...
    for (const item of items)
      await client.query(sql.CREATE_DISTRIBUTION_ITEM, [
        distribution.rows[0].distribution_id,
...
async function listDistributions(req, res) {
  try {
    return res.json({
      distributions: (await pool.query(sql.LIST_DISTRIBUTIONS)).rows,
    });
  } catch (err) {
    console.error("[distributions/list] error:", err);
    return res.status(500).json({ message: "Failed to load distributions" });
  }
}
...
async function listMyDistributions(req, res) {
  try {
    return res.json({
      distributions: (
        await pool.query(sql.LIST_TEAM_DISTRIBUTIONS, [req.user.user_id])
      ).rows,
    });
  } catch (err) {
    console.error("[distributions/mine] error:", err);
    return res
      .status(500)
      .json({ message: "Failed to load assigned distributions" });
  }
}
  ...
  try {
    ...
    if (!isAdmin) {
      const member = await client.query(
        ...
        [current.assigned_team_id, req.user.user_id],
      );
      if (!member.rows[0]) {
        await client.query("ROLLBACK");
        return res
          .status(403)
          .json({ message: "You are not assigned to this distribution" });
      }
    }
    if (current.status === "delivered" || current.status === "cancelled") {
      await client.query("ROLLBACK");
      return res.status(400).json({
        message: "Completed or cancelled distributions cannot be changed",
      });
    }
    if (
      status === "delivered" &&
      !["picked_up", "in_transit"].includes(current.status)
    ) {
      await client.query("ROLLBACK");
      return res
        .status(400)
    ...
    await client.query("ROLLBACK");
    console.error("[distributions/status] error:", err);
    return res.status(500).json({ message: "Failed to update distribution" });
  } finally {
    client.release();
  }
}
